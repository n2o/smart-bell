...
struct LedStrip {
  ...
  struct Color {
    ...
    static var red = Color(r: 16, g: 0, b: 0)
    static var green = Color(r: 0, g: 16, b: 0)
    static var blue = Color(r: 0, g: 0, b: 16)

    var r, g, b: UInt8
  }

  func setPixel(index: Int, color: Color) {
    // Swap R and G because the hardware expects them swapped
    led_strip_set_pixel(
      handle, UInt32(index), UInt32(color.g), UInt32(color.r), UInt32(color.b))
  }
